...
from ReyzuRobot import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        async for i in ubot2.iter_messages(
            "@ReyzuLogoPack", filter=InputMessagesFilterPhotos
        ):
            pics.append(i)
        id_ = random.choice(pics)
        bg_ = await id_.download_media()
        fpath_ = glob.glob("./ReyzuRobot/resources/fonts/*")
    ...
    if os.path.exists(font_):
        if not font_.startswith("./ReyzuRobot/resources/fonts"):
            os.remove(font_)
